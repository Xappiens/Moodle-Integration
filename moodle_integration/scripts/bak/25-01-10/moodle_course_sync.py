...
def process_moodle_course(moodle_instance_name, course_id, api_url, token):
    ...
    try:
        ...
        for participant in participants:
            ...
            # Crear identificador respetando el autoname
            user_instance = moodle_instance_name
            user_id = participant.get("username")

            if not user_id:
                raise ValueError(f"El campo 'username' no está definido para un participante en el curso {course_id}.")

            user_identifier = f"{user_instance} {user_id}"
            ...
            if frappe.db.exists("Moodle User", {"name": user_identifier}):
                ...
                logs.append(f"      Usuario existente actualizado: {user_identifier}")
            else:
                ...
                user_doc.update({
                    "user_id": user_id,  # Asegurando que user_id se configure correctamente
                    "user_name": participant.get("firstname"),
                    "user_surname": participant.get("lastname"),
                    "user_fullname": f"{participant.get('firstname')} {participant.get('lastname')}",
                    "user_email": participant.get("email"),
            ...
            if "student" in roles:
                student_found = False
                for student in course_doc.get("course_students", []):
                    if student.user_student == user_doc.name:
                        student_found = True
                        student.user_group = last_group_name  # Actualizar último grupo asignado
